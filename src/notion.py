...
def process_notion_data(data):
    ...
    if not results:
        return []
    return [{
            "name": properties['properties']['Name']['title'][0]['plain_text'] if properties['properties']['Name']['title'] else '',
            "latitude": properties['properties']['Latitude']['number'],
            "longitude": properties['properties']['Longitude']['number'],
            "status": properties['properties']['Status']['select']['name'] if properties['properties']['Status']['select'] else '',
            "location_type": properties['properties']['Location Type']['select']['name'] if properties['properties']['Location Type']['select'] else '',
